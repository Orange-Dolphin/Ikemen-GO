...
func loadCharPalettes(sff *Sff, filename string, ref int, preload bool) (error) {
	f, err := OpenFile(filename)
	if err != nil {
		return err
	}
...
func preloadSff(filename string, char bool, preloadSpr map[[2]int16]bool) (*Sff, []int32, error) {
	...
	for i := 0; i < len(spriteList); i++ {
		...
		case 1:
			...
			//Check if the sprite uses a shared palette or not
			if i > 0 {
				var ps byte
				if err := read(&ps); err != nil {
					return nil, nil, err
	...
	if sys.usePalette && char{
		loadCharPalettes(sff, filename, len(sys.sel.charlist)-1, true)
	}
